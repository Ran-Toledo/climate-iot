...
| Command shape | Only `type: "set_state"` exists. Payload: `{power?: bool, target_temperature?: int}`, sparse/partial, merged server-side into device state. `target_temperature` was changed from `float` to `int` at Stage 6 (explicitly approved) — no AC hardware supports fractional-degree setpoints, so the API contract was narrowed to match reality rather than the firmware discarding precision nothing could use. |
